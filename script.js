document.addEventListener("DOMContentLoaded", () => {
    ...
    if(dateInput) {
        ...
        dateInput.addEventListener('change', (e) => {
            ...
            if (elDia === 0 || elDia === 1) {
                alert("Los domingos y lunes el local permanece cerrado. Por favor, elegí otro día.");
                e.target.value = "";
...
function generarHorariosDinamicos() {
    ...
    if (numeroDiaSemana >= 2 && numeroDiaSemana <= 6) {
        horasDisponibles = ["11:00", "12:00", "13:00", "14:00", "15:00", "16:00", "17:00", "18:00", "19:00", "20:00"];
    }

    container.innerHTML = '<div class="loading" style="text-align:center;color:#999;padding:10px;">Cargando horarios...</div>';

    tursoQuery('SELECT hora FROM turnos WHERE fecha = ? AND barbero = ?', [fecha, barbero])
        .then(turnos => {
...
function nextStep(stepNumber) {
    if (stepNumber === 2) {
        if (!document.querySelector('input[name="barber"]:checked')) {
            alert("Por favor, seleccioná primero."); return;
        }
    }
    if (stepNumber === 3) {
        if (!document.querySelector('input[name="service"]:checked')) {
            alert("Por favor, seleccioná un servicio primero."); return;
        }
    }
    if (stepNumber === 4) {
        ...
        if (!dateSelected || !timeSelected) {
            alert("Por favor, seleccioná tanto la fecha como la hora."); return;
        }
    }
...
async function confirmarTurno() {
    ...
    if (!pago || !nombre || !telefonoCliente) {
        alert("Por favor, completá todos los datos de contacto."); return;
    }
    ...
    if (!formatoValido) {
        alert("Número de WhatsApp inválido.\n\nIngresalo comenzando con 549.\n\nEjemplo: 5493884418918");
    ...
    try {
        ...
        if (duplicado.length > 0) {
            alert(`El horario de las ${hora} ya fue reservado.`);
            return;
        }

        await tursoQuery(
            `INSERT INTO turnos (cliente, barbero, servicio, fecha, hora, pago, precio, whatsapp_cliente, whatsapp_barbero)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
